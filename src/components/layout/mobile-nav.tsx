        ...
        <div className="grid grid-cols-3 gap-1.5">
          {items.map(({ tab, label, icon: Icon, badge }) => (
            <button
              ...
              aria-label={label}
              className={`relative flex min-h-11 items-center justify-center rounded-2xl transition ${
                mobileTab === tab ? "bg-blue-500/18 text-blue-300" : "text-zinc-500"
              }`}
            >
              <Icon />
              {badge && badge > 0 ? (
                <span className="absolute -top-0.5 right-2 flex h-4 min-w-4 items-center justify-center rounded-full bg-amber-500 px-1 text-[10px] font-bold text-black">
                  {badge}
                </span>
              ) : null}
            </button>
