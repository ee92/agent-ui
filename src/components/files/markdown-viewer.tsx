...
function renderBlock(block: MarkdownBlock, index: number) {
  ...
  if (block.type === "code") {
    return (
      <div key={`code-${index}`} className="overflow-hidden rounded-lg border border-white/[0.06] bg-zinc-800/90">
        {block.language ? (
          <div className="border-b border-white/[0.06] px-4 py-2 text-[10px] uppercase tracking-[0.18em] text-zinc-400">
            {block.language}
          </div>
        ) : null}
  ...
  if (block.type === "hr") {
    return <hr key={`hr-${index}`} className="border-white/[0.06]" />;
  }
